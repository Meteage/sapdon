...
`lr-framework` 是一个面向《我的世界：基岩版》附加包的 L/R 分离开发框架，位于 `examples/lr-framework`。它把生态中反复出现的"连接-源-功能-消费-储量"类系统（电网、管网、信号总线、传力带……）的骨架收敛成一个可继承基类 `BaseEngine`，让运行时脚本统一收在 `scripts/` 下、按 L（逻辑）与 R（渲染）两层组织：L 层是零 `@minecraft/server` 依赖的纯函数结算（图的分段、并网、供/需/储结算），可在 Node 中镜像单测；R 层只负责读世界、写方块状态与事件分发。新系统只需"写一个纯结算 + 继承基类实现约十个成员 + 声明方块 + 一个镜像测试"，即可快速落地。框架自带电力与流体管道两个继承演示（12 个断言全绿），并配套架构说明、快速上手与"从零做第三个系统"的逐步教程，把可验证的领域逻辑与易出错的运行时生命周期彻底解耦。
