import { Entity, world, StartupEvent } from '@minecraft/server';
import {
    BaseComponent,
    ComponentManager,
    Optional,
...
//使用 GameInstance 来创建游戏实例
@GameInstance
export class MyGameInstance extends MinecraftGameInstance {
...
world.afterEvents.playerSpawn.subscribe(({ player }) => {
    const manager = getGameInstance()?.getLevel().addEntity(player.id)
    // 给玩家添加组件, 顺序会影响效果
    manager?.attachComponent?.(
        new MinecraftPlayerInputComponent(),
...
world.afterEvents.playerLeave.subscribe(({ playerId }) => {
    getGameInstance()?.getLevel().removeEntity(playerId)
})

/**
 * 跳跃计数器组件, 用于记录玩家的跳跃次数
 */
class JumpCounterComponent extends BaseComponent<Entity> {
    ...
    protected playerInput = Optional.none<MinecraftPlayerInputComponent>()

    onAttach(manager: ComponentManager<Entity>) {
        // 依赖 `MinecraftPlayerInputComponent`, 但是不需要创建新的 `MinecraftPlayerInputComponent`
        this.playerInput = manager.getComponent(MinecraftPlayerInputComponent)
    }
    ...
    onTick(): void {
        ...
        // 因为 `MinecraftPlayerInputComponent` 一定会创建, 所以这里一定有值
        const input = this.playerInput.unwrap()
        if (input.getKeyPressTimes('Space')) {
            // 每次按下会使跳跃计数器加1
            this.jumpCount++
        }
    }
...
// 二段跳组件, 用于实现二段跳效果
class DoubleJumpComponent extends BaseComponent<Entity> {
    ...
    onTick(): void {
        ...
        const entity = this.getEntity().unwrap()
