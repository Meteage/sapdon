...
- **v3 方块语义（6 面旋转）**：泵/单阀/三通均为 `rotationType: "facing"`（`minecraft:facing_direction`，放置时按玩家朝向，可 6 面放置）。**自身参考系不变**：泵顶=输出底=输入；单阀西入东出；三通西入三出（dir: east/south/west/north 顺时针循环，west=指向输入=全关，顶面单箭头指示）。引擎 `engine/graph.ts` 的 `ROT_FACE` 做局部→世界面 6 向映射（`rotFace(facing, local)` 导出），`facingOf` 读 `minecraft:facing_direction`（兼容旧 `minecraft:cardinal_direction`）。**阀开/关与三通 dir 均为方块状态，describeEnd 必须读旋转做映射，不入持久化表**。泵泡水判定与 `rebuildPumpEnds` 均用 `rotFace(facing, "down")`（局部输入口）。**三通出水口喷水**：`tick.ts` 的 `sprayValve3()` 每 20 tick 检查——输入面管道有水（段 `covered` 含输入管道）且开通面无管道/设备连接 → 出水口喷 `minecraft:water_splash_particle`（方向沿输出面）。
- **方块局部面定义经验（6 面旋转）**：
  - 设备输入/输出口一律用**自身参考系**定义（泵顶=输出底=输入；单阀西入东出；三通西入三出），旋转只决定"模型北面朝哪"——**引擎里绝不写死世界面**，语义面必须经 `rotFace(facing, 局部面)` 映射到世界面。
  - 旋转 trait：`rotationType: "facing"` → `minecraft:facing_direction`（6 向，放置按玩家朝向）；旧 `"cardinal"` → `minecraft:cardinal_direction`（水平 4 向）。框架变换（north=[0,0,0] 为基准）：west=[0,90,0]、south=[0,180,0]、east=[0,-90,0]、up=[90,0,0]（北面仰起朝上）、down=[-90,0,0]（北面前倾朝下）。
  - **ROT_FACE 局部→世界映射表**（`engine/graph.ts`，模型北面朝 facing）：north: N→N S→S E→E W→W U→U D→D；west: N→W S→E E→N W→S U→U D→D；south: N→S S→N E→W W→E U→U D→D；east: N→E S→W E→S W→N U→U D→D；up: N→U S→D E→E W→W U→S D→N；down: N→D S→U E→E W→W U→N D→S。
  - `facingOf` 读 `minecraft:facing_direction`，**必须 fallback 到 `minecraft:cardinal_direction`**——trait 变更后世界里的旧方块状态仍在，漏 fallback 旧存档全部判成默认朝向。
  - **所有"读面/判面"处都要过 `rotFace`**：`describeEnd`（泵顶/底、阀门链 `io.in/io.out`、`terminalEnd` 终端）、`rebuildPumpEnds`（泵输入/输出邻块）、泵泡水判定（局部输入口=down）、三通喷水粒子（输出面）——漏一处就是静默的面判反 bug。
  - **`textures_arr` 面顺序 = `[up, down, east, west, south, north]`**（框架 `Fe` 基类固定分配，不是常规 `[up,down,north,south,east,west]`）——贴图数组按此顺序放（泵 `["output","input","off",...]`，单阀 `["v3t_east","v_body","output","input",...]`）。
  - 几何旋转（transformation）整体转模型，贴图自动跟随，无需手动换贴图面；视觉符号（output/input 标）画在**模型局部参考系**对应面即可，旋转后自然朝世界 facing。
