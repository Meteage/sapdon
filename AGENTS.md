...
| 带实体方块 | `src/core/factory/blockFactory.js` | `BlockAPI.createTileBlock(identifier, category, textures_arr, options)` → 注册方块 + 实体（behavior/resource）；**★ 这是当前唯一可用的方块容器路线**，`options` 可带 `inventory_size` / `container_type` / `can_be_siphoned_from`（默认 27 / `minecart_chest` / true，不传 = 产物逐字节不变；每次构造按实例拷贝）+ `group` / `hide_in_command` / `format_version` / `entity_texture`（★ S3b 补：这四个以前 `.d.ts` 里漏声明，传对象字面量会踩 TS2353）+ **`despawn_delay`**（★ 2026-09-12 补：承载实体 despawn 的**延迟秒数**，写到 `item_despawn` 组 `minecraft:transformation.delay.value`；不传 = 不写该键 ⇒ 产物不变。用途见 `known-pitfalls.md` §4.15：`drop_inventory: true` 是**整容器一起倒出来**、**没有**按格过滤的字段，破坏时要清内部格的项目靠它开时间窗） |
