...
export class ServerFormButton extends UIElement {
   ...
   * 定义一个新的表单按钮
   * @param {string} name
   * @param {string} binding_button_name
   */
  constructor(name: string, binding_button_name: string) {
...
form_button_panel.dataBinding.addDataBinding(
  new DataBindingObject().setBindingName('#form_button_length').setBindingNameOverride('#collection_length')
)
...
export class ServerUISystem {
  static #binding_map: Map<string, string> = new Map()

  static #binding_title_list: string[] = []

  static addBindingTitle(title_name: string): void {
    this.#binding_title_list.push(title_name)
  }

  static getBindingTitleList(): string[] {
    return this.#binding_title_list.concat(Array.from(this.#binding_map.keys()))
  }

  static getBindingContentList(): string[] {
    return Array.from(this.#binding_map.values())
  }

  static bindingTitlewithContent(title_name: string, content: string): void {
    // 添加绑定
    this.#binding_map.set(title_name, content)
    this.#updateServerFormSystem()
  }

  static #updateServerFormSystem(): void {
    const user_custom_ui_text = this.getBindingTitleList()
      .map((title_name) => {
    ...
    const long_form = new UIElement('long_form').addModification({
      array_name: 'bindings',
    ...
    custom_server_form_factory.factory
      .setName('server_form_factory')
      .setControlIds({
    ...
    if (!ServerFormSystem.getElement('custom_ui_template')) {
      ...
      custom_ui_template.dataBinding
        .addDataBinding(new DataBindingObject().setBindingName('#title_text'))
        .addDataBinding(
          new DataBindingObject().setBindingType('view')
    ...
    if (!ServerFormSystem.getElement('main_screen_content')) {
      const main_screen_content = new UIElement('main_screen_content').addModification({
        array_name: 'controls',
        operation: Modifications.OPERATION.INSERT_BACK,
        value: [custom_server_form_factory.serialize()],
      })
    ...
    ServerFormSystem.addElement(long_form).addElement(custom_root_panel)
  }
}