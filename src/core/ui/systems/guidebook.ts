...
const page_template = new StackPanel('page_template')
...
export class Guidebook {
  #bindingList: { page_name: string; left_control_name: string; right_control_name: string }[] = []
  system: UISystem
  ...
  addPageBinding(page_name: string, left_control_name: string, right_control_name: string): this {
  ...
  addPage(page_name: string, left_control: UIElement, right_control: UIElement): this {
  ...
  #updateSystem(): void {
    ...
    guidebook_root_panel.addControls([
      ...
      // 添加书页
      new StackPanel('book_page_stack_panel')
        .setOrientation('horizontal')
      ...
      new Panel('page_content_root')
        ...
        .setLayout(new Layout().setSize(['90%', '90%']))
        .addControls([
          new Panel('label_text_panel')
            .setLayout(
              new Layout().setSize([32, 8]).setAnchorFrom('bottom_middle').setAnchorTo('bottom_middle')
            )
            .addControl(
              new Label('page_content').setText(new Text().setText('#form_text').setColor([0, 0, 0]))
            ),
      ...
      // 添加关闭按钮
      new Button('close_button')
        .setLayout(
          new Layout().setSize([14, 14]).setAnchorFrom('top_right').setAnchorTo('top_right')
        )
        .setInput(
      ...
      new Panel('prev_button_panel')
        ...
        .setControl(new Control().setLayer(5))
        .setLayout(
          new Layout().setSize([24, 24]).setOffset([7, -9]).setAnchorFrom('bottom_left').setAnchorTo('bottom_left')
        )
        .addControl(
      ...
      new Panel('next_button_panel')
        ...
        .setControl(new Control().setLayer(5))
        .setLayout(
          new Layout().setSize([24, 24]).setOffset([-7, -9]).setAnchorFrom('bottom_right').setAnchorTo('bottom_right')
        )
        .addControl(
