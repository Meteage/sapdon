...
const custom_form_root = new Panel("custom_form_root")
      ...
      custom_form_root.dataBinding.addDataBinding(
        new DataBindingObject().setBindingType("view")
        .setSourceControlName("custom_panel")
        .setSourcePropertyName("(not ((#title_text - $title_needs_to_contain) = #title_text))")
        .setTargetPropertyName("#visible")
      );
     
//加入用户自定义UI
user_custom_ui_list.forEach(({form_name,ui_system})=>{
    const custom_ui_root_panel = new Panel(form_name)
          custom_ui_root_panel.control.addControl(
            ui_system.serialize()
          )
    custom_form_root.control.addControl(custom_ui_root_panel.serialize())
})
...
const ServerFromSystem = new UISystem("server_form:server_form","ui/")
.addElement(long_form)
.addElement(custom_root_panel)
.addElement(main_screen_content)

 return ServerFromSystem;
}
