...
export class Sprite {
  ...
  declare allow_debug_missing_texture: boolean
  declare uv: UV
  declare uv_size: UV
  declare texture_file_system: TextureFileSystem
  declare nineslice_size: number | [number, number, number, number]
  declare tiled: boolean | 'x' | 'y'
  declare tiled_scale: [number, number]
  declare clip_direction: ClipDirection | string
  declare clip_ratio: number
  declare clip_pixelperfect: boolean
  declare keep_ratio: boolean
  declare bilinear: boolean
  declare fill: boolean
  declare $fit_to_width: boolean
  declare zip_folder: string
  declare grayscale: boolean
  declare force_texture_reload: boolean
  declare base_size: [number, number]
   ...
   * @returns {Sprite} 返回当前实例以支持链式调用
   */
  setAllowDebugMissingTexture(allow = true): this {
   ...
   * @returns {Sprite} 返回当前实例以支持链式调用
   */
  setTextureFileSystem(source: TextureFileSystem = 'InUserPackage'): this {
   ...
   * @returns {Sprite} 返回当前实例以支持链式调用
   */
  setTiledScale(scale: [number, number] = [1, 1]): this {
   ...
   * @returns {Sprite} 返回当前实例以支持链式调用
   */
  setClipPixelPerfect(pixelPerfect = false): this {
   ...
   * @returns {Sprite} 返回当前实例以支持链式调用
   */
  setBilinear(bilinear = false): this {
   ...
   * @returns {Sprite} 返回当前实例以支持链式调用
   */
  setGrayscale(grayscale = false): this {
   ...
   * @returns {Sprite} 返回当前实例以支持链式调用
   */
  setForceTextureReload(force = false): this {
