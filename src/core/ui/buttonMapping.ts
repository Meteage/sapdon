...
// 注意：本类在构造器中真赋默认值（own 枚举属性会按现有行为被序列化），与其它 property 包（空构造）不同。
export class ButtonMapping {
   ...
   * @returns {ButtonMapping} 返回当前实例以支持链式调用
   */
  setIgnoreInputScope(ignore = false): this {
   ...
   * @returns {ButtonMapping} 返回当前实例以支持链式调用
   */
  setHandleSelect(handle = false): this {
   ...
   * @returns {ButtonMapping} 返回当前实例以支持链式调用
   */
  setHandleDeselect(handle = false): this {
   ...
   * @returns {ButtonMapping} 返回当前实例以支持链式调用
   */
  setButtonUpRightOfFirstRefusal(handle = false): this {
