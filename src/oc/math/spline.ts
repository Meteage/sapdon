// Modified from https://github.com/likr/spline-interpolator

import { bisectRight } from 'd3-array'

const quincunx = (u: number[], v: number[], w: number[], q: number[]) => {
    const n = u.length - 1
...
const smoothingSpline = (x: number[], y: number[], sigma: number[], lambda: number) => {
    const n = x.length - 1
...
export class SplineInterpolator {
    ...
    fromPoints(points: Point[], lambda = 1) {
        const x = points.map(p => p.x)
        const y = points.map(p => p.y)
        return new SplineInterpolator(x, y, lambda)
    }

    constructor(xIn: number[], yIn: number[], lambda = 1) {
    ...
    curve(nInterval: number, domain?: [number, number]) {
        domain = domain ?? this.domain() as [number, number]
